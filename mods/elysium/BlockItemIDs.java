package mods.elysium;

import net.minecraft.item.Item;

import org.lwjgl.input.Keyboard;

public class BlockItemIDs
{
	public static final String modId = "elysium";
	public static final String NET_CHANNEL_NAME = "elysium";
	public static int DimensionID = 8;
	public static int MAX_DRAGON_IN_END = 1;
	
	public static int idPalestoneBlock = 200;
	public static int idSoilBlock = 201;
	public static int idGrassBlock = 202;
	public static int idLeucosandBlock = 203;
	public static int idRiltBlock = 204;
	public static int idWaterFlowingBlock = 205;
	public static int idWaterBlock = 206;
	
	public static int idPortalCoreBlock = 210;
	
	public static int idFostimberSaplingBlock = 2020;
	public static int idFostimberLogBlock = 2021;
	public static int idFostimberLeavesBlock = 2022;
	
	public static int idGastroShellBlock = 2023;
	
	public static int idWoodBlock = 2024;
	
	public static int idAsphodelFlowerBlock = 2025;
	public static int idCurlgrassBlock = 2026;
	
	public static int idOreSulphurBlock = 2027;
	public static int idOreCobaltBlock = 2028;
	public static int idOreIridiumBlock = 2029;
	public static int idOreSiliconBlock = 2030;
	public static int idOreJadeBlock = 2031;
	public static int idOreTourmalineBlock = 2032;
	public static int idOreBerylBlock = 2033;
	
	public static int idShellsBlock = 2034;
	public static int idConchBlock = 2035;
	
	public static int idPaleCobblestone = 2036;
	public static int idPaleCobblestoneMossy = 2037;
	public static int idPalestoneBrick = 2038;
	public static int idPalestoneBrickCracked = 2039;
	public static int idPalestoneBrickMossy = 2040;
	public static int idPalestoneBrickChiseld = 2041;
	public static int idPalestonePillar = 2042;
	
	public static int idExpeller = 2043;
	
	public static int idBlockSulphur = 2044;
	public static int idBlockCobalt = 2045;
	public static int idBlockIridium = 2046;
	public static int idBlockSilicon = 2047;
	public static int idBlockJade = 2048;
	public static int idBlockTourmaline = 2049;
	public static int idBlockBeryl = 2050;
	
	//Items
	public static int idGracePrismItem = 5000;
	public static int idWhistleItem = 5001;
	
	public static int idPepperSeedItem = 5002;
	public static int idAsphodelPetalsItem = 5003;
	
	public static int idBerylItem = 5004;
	public static int idCobaltIngotItem = 5005;
	public static int idIridiumIngotItem = 5006;
	public static int idJadeItem = 5007;
	public static int idSiliconChunk = 5008;
	public static int idSulphurItem = 5009;
	public static int idTourmalineItem = 5010;
	public static int idSturdyHideItem = 5011;
	
	public static int idWoodSwordItem = 5051;
	public static int idWoodPickaxeItem = 5052;
	public static int idWoodAxeItem = 5053;
	public static int idWoodShovelItem = 5054;
	public static int idWoodHoeItem = 5055;
	
	public static int idStoneSwordItem = 5056;
	public static int idStonePickaxeItem = 5057;
	public static int idStoneAxeItem = 5058;
	public static int idStoneShovelItem = 5059;
	public static int idStoneHoeItem = 5060;
	
	public static int idElysianWaterBucket = 5110;

	public static int idOverkillItem = 5555;
	public static int idDebugItem = 5107;

	//Settings
	public static int maxportaldistance = 64;
	public static byte ticksbeforeportalcheck = 5;
	public static byte ticksbeforeportalteleport = 100;
}
