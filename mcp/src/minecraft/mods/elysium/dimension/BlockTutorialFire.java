...
public class BlockTutorialFire extends BlockFire {
		...
		if (par1World.getGameRules().getGameRuleBooleanValue("doFireTick")) {
			...
			if ((!flag)
				...
				if ((!flag) && (!canNeighborBurn(par1World, par2, par3, par4))) {
					if ((!par1World.doesBlockHaveSolidTopSurface(par2,
							par3 - 1, par4)) || (l > 3)) {
						par1World.setBlockToAir(par2, par3, par4);
					}
				} else if ((!flag)
	...
	public void onBlockAdded(World par1World, int par2, int par3, int par4) {
		...
		if ((par1World.getBlockId(par2, par3 - 1, par4) != Block.blockDiamond.blockID)
				|| (!Elysium.TutorialPortal.tryToCreatePortal(par1World, par2,
						par3, par4))) {
			if ((!par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1, par4))
					&& (!canNeighborBurn(par1World, par2, par3, par4))) {
				par1World.setBlockToAir(par2, par3, par4);
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if ((!par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1, par4))
				&& (!Elysium.TutorialFire.canBlockCatchFire(par1World, par2,
						par3 - 1, par4, ForgeDirection.UP))) {
			if (Elysium.TutorialFire.canBlockCatchFire(par1World, par2 - 1, par3,
			...
			if (Elysium.TutorialFire.canBlockCatchFire(par1World, par2, par3,
			...
			if (Elysium.TutorialFire.canBlockCatchFire(par1World, par2, par3 + 1,
					par4, ForgeDirection.DOWN)) {
				for (int l = 0; l < 2; l++) {
					float f = par2 + par5Random.nextFloat();
	...
	public void registerIcons(IconRegister par1IconRegister) {
		this.iconArray = new Icon[] {
				par1IconRegister.registerIcon("Tutorial:Tutorialfire_0"),
				par1IconRegister.registerIcon("Tutorial:Tutorialfire_1") };
	}
