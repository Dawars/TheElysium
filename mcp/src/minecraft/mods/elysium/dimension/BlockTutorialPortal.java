...
public class BlockTutorialPortal extends BlockBreakable {
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3,
		...
		byte b0 = 0;
		byte b1 = 0;
		if (par1World.getBlockId(par2 - 1, par3, par4) == Block.blockDiamond.blockID
				|| par1World.getBlockId(par2 + 1, par3, par4) == Block.blockDiamond.blockID) {
			b0 = 1;
		}
		if (par1World.getBlockId(par2, par3, par4 - 1) == Block.blockDiamond.blockID
				|| par1World.getBlockId(par2, par3, par4 + 1) == Block.blockDiamond.blockID) {
			b1 = 1;
		}
		if (b0 == b1) {
			return false;
			...
			for (l = -1; l <= 2; ++l) {
				for (i1 = -1; i1 <= 3; ++i1) {
					...
					if (l != -1 && l != 2 || i1 != -1 && i1 != 3) {
						...
						} else if (j1 != 0 && j1 != Elysium.TutorialFire.blockID) {
							return false;
						}
					}
				}
			...
			for (l = 0; l < 2; ++l) {
				for (i1 = 0; i1 < 3; ++i1) {
					par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l,
							Elysium.TutorialPortal.blockID, 0, 2);
				}
			}
			return true;
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (par1World.getBlockId(par2, i1 - 1, par4) != Block.blockDiamond.blockID) {
			...
			if (j1 == 3
					&& par1World.getBlockId(par2, i1 + j1, par4) == Block.blockDiamond.blockID) {
				...
				if (flag && flag1) {
					...
					if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != Block.blockDiamond.blockID || par1World
							.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID)
							&& (par1World
									.getBlockId(par2 - b0, par3, par4 - b1) != Block.blockDiamond.blockID || par1World
									.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID)) {
						par1World.setBlockToAir(par2, par3, par4);
					}
				}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3,
		...
		if ((par5Entity.ridingEntity == null)
			...
			} else if (thePlayer.dimension != Elysium.DimensionID) {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager()
						.transferPlayerToDimension(
								thePlayer,
								Elysium.DimensionID,
								new TutorialTeleporter(thePlayer.mcServer
										.worldServerForDimension(Elysium.DimensionID)));
			} else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager()
						.transferPlayerToDimension(
